// ============================================================
// ONE-CLICK ACTIONS: CORE LOGIC AND UI
// ============================================================
...
function executeQuickAction(actionId) {
  ...
  switch (actionId) {
    ...
    case "debt-snowball":
      ...
      break;
    default:
      return { success: false, title: "Unknown Action", message: "Unknown action", type: "error" };
  }
...
function executeGoalBoost(snapshot) {
  ...
  return {
    ...
    details: [
      ...
      `Progress: ${progress}%`,
      topGoal.saved >= topGoal.target
        ? "Goal completed!"
        : `${formatCurrency(topGoal.target - topGoal.saved)} remaining`,
    ],
  };
}
...
function executeDebtSnowball(snapshot) {
  ...
  return {
    ...
    details: [
      ...
      extraPayment > 0
        ? `Suggested extra payment: ${formatCurrency(extraPayment)}/month (30% of surplus)`
        : null,
...
// Expose actions functions globally for cross-module access
